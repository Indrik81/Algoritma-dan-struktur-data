...
# Konstanta
WIDTH, HEIGHT = 600, 700  # Lebar dan tinggi window
LINE_WIDTH = 15
BOARD_ROWS = 3
BOARD_COLS = 3
...
try:
    ...
    print("Sound files not found. Please ensure 'win.wav', 'lose.wav', and 'click.wav' are in the same directory.")
    win_sound = None
    click_sound = None
...
while running:
    ...
    # Jika dalam mode input, gambar prompt
    if input_mode:
        prompt_text = f"Masukkan nama untuk Player {input_for} (X):" if input_for == 1 else f"Masukkan nama untuk Player {input_for} (O):"
        ...
        if draw_button("Set Names", 350, 10, 120, 40, BUTTON_COLOR, BUTTON_HOVER_COLOR):
            ...
            input_for = 1
            current_input = ""
        if draw_button("Quit", 500, 10, 80, 40, BUTTON_COLOR, BUTTON_HOVER_COLOR):
            running = False
